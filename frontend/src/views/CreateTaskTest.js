...
class CreateTaskTest extends React.Component {
    ...
    deleteSelectedUsers = (userName, userId) => {
        ...
        if (this.state.editUserDeleteIds.length === 0) {
            ...
            for (let i = 0; i < this.state.usersTemp.length; i++) {
                for (let j = 0; j < this.state.editUserDeleteIds.length; j++) {
                    if (this.state.usersTemp[i].userId === this.state.editUserDeleteIds[j]) {
                        returnFlag = 1;
                        break;
                    } else {
                        returnFlag = 0;
            ...
            for (let i = 0; i < this.state.userObj.length; i++) {
                for (let j = 0; j < this.state.editUserDeleteIds.length; j++) {
                    if (this.state.userObj[i].userId === this.state.editUserDeleteIds[j]) {
                        returnFlag = 1;
                        break;
                    } else {
                        returnFlag = 0;
        ...
        if (returnFlag === 0) {
            this.setState((prevState) => ({ editUserDeleteIds: [...prevState.editUserDeleteIds, userId], users: [...prevState.users, filteredArray1[0]], userObj: filteredArray }))
        }
